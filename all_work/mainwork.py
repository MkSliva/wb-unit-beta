...
# Эта функция теперь будет брать purchase_price из таблицы sales,
# если активная партия не найдена для соответствующей даты и vendor_code.

# ... (весь ваш существующий код, включая send_telegram_notification) ...

@app.get("/api/test_telegram_notification")
async def test_telegram_notification(message: str = Query("Test message from FastAPI backend!", description="Message to send to Telegram")):
    """
...
def get_sales_grouped_detailed_range(
    ...
    try:
        ...
        # ⚠️ ОБЯЗАТЕЛЬНО: Выбираем "purchase_price" из таблицы sales как исходное значение
        query = """
                SELECT "imtID", "vendorCode", "ordersCount", "ad_spend", "actual_discounted_price",
                       "purchase_price", -- <--- Добавлен или убедиться, что он есть
                       "delivery_to_warehouse", "wb_commission_rub",
                       "wb_logistics", "tax_rub", "packaging", "fuel", "gift", "defect_percent",
                       date -- Дата нужна для сопоставления с партиями
                FROM sales
        ...
        # --- ИЗМЕНЕНИЕ: ОБНОВЛЯЕМ purchase_price НА ОСНОВЕ ПАРТИЙ ИЛИ sales.purchase_price ---
    ...
    try:
        ...
        query = """
            SELECT date, "ordersCount", "ad_spend", "salePrice",
                   ...
                   "delivery_to_warehouse", "wb_commission_rub",
                   "wb_logistics", "tax_rub", "packaging", "fuel", "gift", "defect_percent",
                   "actual_discounted_price", "vendorCode"
            FROM sales
...
### Ежедневные продажи по конкретной imtID
...
async def create_purchase_batch(batch: PurchaseBatchCreate):
    ...
    try:
        ...
        cursor.execute(
            ...
            WHERE vendor_code = %s AND is_active = TRUE AND (end_date IS NULL OR end_date >= %s);
            """,
...
def update_costs(update: CostUpdate):
    ...
    try:
        ...
        updatable_fields = [
            "purchase_price", # <--- Оставляем purchase_price здесь, если хотим, чтобы этот эндпоинт тоже мог его обновлять
            "delivery_to_warehouse", "wb_commission_rub",
            "wb_logistics", "tax_rub", "packaging", "fuel", "gift", "defect_percent"
        ]
        ...
        for field in updatable_fields:
            ...
            # numpy.isnan() корректно обрабатывает NaN, в отличие от простого 'is not None' для float
            # Проверяем, что val не является None и не является NaN
            if val is not None and not (isinstance(val, float) and np.isnan(val)):
                fields_to_update_sales.append(f'"{field}" = %s') # Используем двойные кавычки для названий столбцов
                values_to_update_sales.append(val)
                fields_to_update_cards.append(f'"{field}" = %s')
                values_to_update_cards.append(val)
            # Если val является NaN (пришло с фронтенда как null, а затем FastAPI преобразовал в NaN для Optional[float])
        ...
        if fields_to_update_sales:
            ...
            # Используем "vendorCode" для sales (как в вашей БД)
            query_sales_update = f"UPDATE sales SET {', '.join(fields_to_update_sales)} WHERE \"vendorCode\" = %s AND date >= %s"
            cursor.execute(query_sales_update, values_to_update_sales + [update.vendorcode, update.start_date.strftime('%Y-%m-%d')])
            updated_rows_sales = cursor.rowcount

            # Пересчитываем cost_price и total_profit в таблице sales
            # Используем COALESCE для всех полей, чтобы NULL трактовались как 0 в расчетах
            cursor.execute(
                """
                UPDATE sales
            ...
            if fields_to_update_cards: # Убедимся, что есть поля для обновления
                # ИСПРАВЛЕНИЕ: "vendorCode" изменено на "vendor_code" в таблице cards
        ...
        raise e # Перевыбрасываем ошибку, чтобы её обработал @app.exception_handler
    finally:
        if cursor:
            cursor.close()
        if conn:
...
async def check_purchase_batches():
    ...
    try:
        ...
        # Выбираем активные партии и суммируем ordersCount для каждого vendor_code
        # с даты начала партии
        query = """
            SELECT
                pb.id AS batch_id,
                pb.vendor_code,
            ...
            WHERE
                pb.is_active = TRUE
            GROUP BY
                pb.id, pb.vendor_code, pb.purchase_price, pb.quantity_bought
            HAVING
                pb.quantity_bought <= COALESCE(SUM(s."ordersCount"), 0)
            ;
        """
        ...
        for batch_id, vendor_code, purchase_price, quantity_bought, total_orders_since_batch_start in batches_to_deactivate:
            ...
            cursor.execute(
                """
                UPDATE purchase_batches
                SET is_active = FALSE, end_date = %s, updated_at = CURRENT_TIMESTAMP
                WHERE id = %s;
                """,
